...
var _newly_connected_clients: Array[WebSocketPeer] = []
...
func _process(_delta: float) -> void:
	...
	if _server and _server.is_connection_available():
		...
		ws_peer.inbound_buffer_size = 16777216
		ws_peer.outbound_buffer_size = 16777216
		ws_peer.accept_stream(peer)
		_newly_connected_clients.append(ws_peer)
		if DEBUG_MODE: print("[GodotMaker Bridge] Incoming connection handshaking...")

	# 处理正在握手的连接
	var opened_indices = []
	for i in range(_newly_connected_clients.size()):
		var client = _newly_connected_clients[i]
		client.poll()
		var state = client.get_ready_state()
		if state == WebSocketPeer.STATE_OPEN:
			_clients.append(client)
			opened_indices.append(i)
			if DEBUG_MODE: print("[GodotMaker Bridge] New agent connected and OPEN.")
			agent_connected.emit()
		elif state == WebSocketPeer.STATE_CLOSED:
			opened_indices.append(i)
	
	opened_indices.reverse()
	for i in opened_indices:
		_newly_connected_clients.remove_at(i)
